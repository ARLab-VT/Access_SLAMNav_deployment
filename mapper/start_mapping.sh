...
if [ "$ACTION" = "help" ]; then
	...
	if [ "$ACTION" = "bag" ]; then
		...
		tmux send-keys -t rosbagger "rosbag record --lz4 -o '$OUT' /zed/zed_node/rgb/image_rect_color /zed/zed_node/rgb/camera_info /zed/zed_node/depth/camera_info /zed/zed_node/depth/depth_registered /zed/zed_node/odom /tf" C-m
